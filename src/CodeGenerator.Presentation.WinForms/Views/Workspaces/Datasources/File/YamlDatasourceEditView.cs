...
public partial class YamlDatasourceEditView : UserControl, IView<YamlDatasourceEditViewModel>
{
    ...
    private void ViewModel_PropertyChanged(object? sender, PropertyChangedEventArgs e)
    {
        ...
        if (e.PropertyName == nameof(YamlDatasourceEditViewModel.IsLoadingFile))
        {
            UpdateUI();
        }
        else if (e.PropertyName == nameof(YamlDatasourceEditViewModel.StatusMessage))
        {
            lblStatus.Text = _viewModel?.StatusMessage ?? string.Empty;
        }
        else if (e.PropertyName == nameof(YamlDatasourceEditViewModel.ErrorMessage))
        {
            lblError.Text = _viewModel?.ErrorMessage ?? string.Empty;
            lblError.Visible = !string.IsNullOrEmpty(_viewModel?.ErrorMessage);
        }
        else if (e.PropertyName == nameof(YamlDatasourceEditViewModel.FileInfo))
        {
            RefreshFileInfo();
        }
    }
    ...
    private async void BtnImport_Click(object? sender, EventArgs e)
    {
        ...
        {
            MessageBox.Show($"Error importing YAML: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }
