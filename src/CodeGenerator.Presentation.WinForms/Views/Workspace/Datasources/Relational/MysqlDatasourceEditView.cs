...
namespace CodeGenerator.Presentation.WinForms.Views
{
    ...
    public partial class MysqlDatasourceEditView : UserControl, IView<MysqlDatasourceEditViewModel>
    {
        ...
        private void ViewModel_PropertyChanged(object? sender, PropertyChangedEventArgs e)
        {
            ...
            if (e.PropertyName == nameof(MysqlDatasourceEditViewModel.IsConnecting))
            {
                UpdateUI();
            }
            else if (e.PropertyName == nameof(MysqlDatasourceEditViewModel.ConnectionStatus))
            {
                lblStatus.Text = _viewModel?.ConnectionStatus ?? string.Empty;
            }
            else if (e.PropertyName == nameof(MysqlDatasourceEditViewModel.ErrorMessage))
            {
                lblError.Text = _viewModel?.ErrorMessage ?? string.Empty;
                lblError.Visible = !string.IsNullOrEmpty(_viewModel?.ErrorMessage);
            }
            else if (e.PropertyName == nameof(MysqlDatasourceEditViewModel.SelectedObject))
            {
