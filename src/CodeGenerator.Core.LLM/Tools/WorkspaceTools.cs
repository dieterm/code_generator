...
namespace CodeGenerator.Core.LLM.Tools
{
    ...
    /// and registered through LlmOperationBridge.
    /// </summary>
    public class WorkspaceTools
    {
        ...
        public string ListScopes()
        {
            ...
            {
                var workspace = GetWorkspace();
                var scopes = workspace.GetAllScopes(false, true).Select(s => $"Id: '{s.Id}' Name: '{s.Name}'").ToList();
        ...
        {
            var dataTypes = GenericDataTypes.All.Select(dt => dt.Id).ToArray();
