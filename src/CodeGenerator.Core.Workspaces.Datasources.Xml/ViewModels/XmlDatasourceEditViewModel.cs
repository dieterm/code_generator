...
/// <summary>
/// ViewModel for editing XML datasource properties
/// </summary>
public class XmlDatasourceEditViewModel : ViewModelBase
{
    private readonly XmlSchemaReader _schemaReader;
    private XmlDatasourceArtifact? _datasource;
    private bool _isLoading;
    private bool _isLoadingFile;
    private string? _statusMessage;
    private string? _errorMessage;
    private XmlFileInfoViewModel? _fileInfo;

    public XmlDatasourceEditViewModel()
    {
        _schemaReader = new XmlSchemaReader();
    ...
    /// </summary>
    public XmlDatasourceArtifact? Datasource
    {
        get => _datasource;
        set
        {
    ...
    {
        if (e.PropertyName == nameof(XmlDatasourceArtifact.Name) && !_isLoading)
        {
    ...
    /// </summary>
    public XmlFileInfoViewModel? FileInfo
    {
        get => _fileInfo;
    ...
    private void SaveToDatasource()
    {
        ...
        _datasource.Name = NameField.Value?.ToString() ?? "XML Datasource";
        _datasource.FilePath = FilePathField.Value?.ToString() ?? string.Empty;
    }

    /// <summary>
    /// Load file info from the XML file
    /// </summary>
    public async Task LoadFileInfoAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            ErrorMessage = "Please select an XML file first.";
            return;
        }
        ...
        try
        {
            var info = await _schemaReader.GetFileInfoAsync(filePath, _datasource.RootElementPath, cancellationToken);
            FileInfo = XmlFileInfoViewModel.FromXmlFileInfo(info);

            if (info.HasRepeatingElements)
            {
                StatusMessage = $"Found {info.ItemCount} '{info.RepeatingElementName}' elements with {info.PropertyCount} properties";
            }
            else
            {
                StatusMessage = $"Root element '{info.RootElementName}' with {info.PropertyCount} properties detected";
            }
        }
        catch (Exception ex)
        {
    ...
    /// <summary>
    /// Import the XML file as a table
    /// </summary>
    public async Task ImportTableAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var table = await _schemaReader.ImportXmlAsync(
                filePath,
                _datasource.Name,
                _datasource.RootElementPath,
                cancellationToken);
